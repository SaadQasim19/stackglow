...
const LIST_SECURITY_HEADERS = JSON_SECURITY_HEADERS;

/** Human-readable display names for icon slugs — allocated once at module load. */
const CUSTOM_DISPLAY_NAMES: Record<string, string> = {
  ...
  ngrxselectors: "NgRx Selectors",
  "ngrx-selectors": "NgRx Selectors",
  ngrxstate: "NgRx State",
  ...
  bitcoin: "Bitcoin",
  btc: "Bitcoin",
  earthengine: "Google Earth Engine",
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const displayNames = CUSTOM_DISPLAY_NAMES;
    ...
    for (const file of svgFiles) {
      ...
      if (!processedIds.has(cleanId)) {
        ...
        const displayName =
          displayNames[cleanId] ||
          displayNames[lowerBase] ||
          cleanId
            .replace(/[_-]/g, " ")
    ...
    // Error handled: return empty list as safe fallback
    return NextResponse.json([], { headers: LIST_SECURITY_HEADERS });
  }
}
